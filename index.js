...
app.post("/order", myRoute, (request, response) => {
    const id = uuid.v4();
    const { order, clientName: clientName, price } = request.body;
    orders.push({id, order, clientName: clientName, price, status: "Em preparação"});

    return response.status(201).json({id, clientName: clientName, price, status: "Em preparação"});
});
